...
def find_common_ancestor_in_binary_tree(a_node: TreeNode, b_node: TreeNode) -> Optional[TreeNode]:
    a_ancestors: Dict[TreeNode] = {a_node.value: a_node}
    a_node_to_process: TreeNode = a_node
    while a_node_to_process.parent is not None:
        a_node_to_process = a_node_to_process.parent
        a_ancestors[a_node_to_process.value] = a_node_to_process
    b_node_to_process: TreeNode = b_node
    while b_node_to_process is not None:
        if b_node_to_process.value in a_ancestors:
