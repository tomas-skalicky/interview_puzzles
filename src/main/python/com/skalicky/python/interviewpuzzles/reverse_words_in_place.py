...
def rotate_substring_to_left(words: List[str], rotation_length: int, substring_start_index: int,
    ...
    for i in range(0, rotation_length):
        ...
        while current_index not in indices_with_rotated_values:
...
def reverse_words_in_place(input_characters: List[str]) -> None:
    left_word_start_index: int = 0
    left_word_end_index_included: int = left_word_start_index
    right_word_end_index_included: int = len(input_characters) - 1
    right_word_start_index: int = right_word_end_index_included
    while True:
        while left_word_end_index_included < right_word_start_index and input_characters[left_word_end_index_included] != ' ':
        ...
        if left_word_end_index_included >= right_word_start_index:
            ...
            left_word_end_index_included -= 1
            while right_word_start_index > left_word_end_index_included and input_characters[right_word_start_index] != ' ':
            ...
            if left_word_end_index_included >= right_word_start_index:
                ...
                if left_word_length == right_word_length:
                    swap_substrings_in_place(input_characters, left_word_start_index, left_word_end_index_excluded,
                                             right_word_start_index)
                    ...
                    swap_substrings_in_place(input_characters, left_word_start_index + length_difference,
                    ...
                    rotate_substring_to_left(input_characters, length_difference, left_word_start_index,
                    ...
                    rotate_substring_to_left(input_characters, right_word_length, right_word_start_index - length_difference,
                                             right_word_end_index_excluded)
                else:
                    # Rotates a substring by ${left_word_length} to the left.
                    # Example: "foo to blabla YOURS bar" -> "foo blabla YOURSto bar"
                    rotate_substring_to_left(input_characters, left_word_length, left_word_start_index,
                                             right_word_end_index_excluded)
                    ...
                    rotate_substring_to_left(input_characters, right_word_start_index - left_word_end_index_excluded,
                                             left_word_start_index, right_word_end_index_excluded - left_word_length)

                left_word_start_index += right_word_length + 1
                left_word_end_index_included = left_word_start_index
                right_word_end_index_included -= (left_word_length + 1)
