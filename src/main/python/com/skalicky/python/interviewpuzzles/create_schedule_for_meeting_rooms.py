...
def create_schedule_for_meeting_rooms(meeting_times: List[Tuple[int, int]]) -> int:
    ...
    for meeting_times in sorted_meeting_times:
        if meeting_times[0] != meeting_times[1]:
            if len(sorted_rooms_by_end_of_last_meeting_asc) == 0:
                new_room: RoomAllocation = RoomAllocation(meeting_times[0], meeting_times[1])
                ...
                if room_with_least_end.end_of_last_meeting <= meeting_times[0]:
                    room_with_least_end.end_of_last_meeting = meeting_times[1]
